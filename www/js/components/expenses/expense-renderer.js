/**
 * Expense Renderer Module - SPLIT DESKTOP / MOBILE
 * Desktop: Timeline con griglia 5 colonne e date block grandi
 * Mobile: Lista compatta card-based, ottimizzata per touch
 */
const ExpenseRenderer = {
    ...
    renderList(expenses) {
        ...
        return this.isMobile()
            ? this._renderMobile(groupedByDate)
            : this._renderDesktop(groupedByDate);
    },

    // ============================================================
    //  DESKTOP RENDER (originale, griglia 5 colonne)
    // ============================================================
    _renderDesktop(groupedByDate) {
        ...
        Object.entries(groupedByDate).forEach(([dateStr, dayExpenses]) => {
            ...
            const dayTotal = this._calcDayTotal(dayExpenses);
            ...
            html += `
                <div class="relative pl-24 animate-fadeIn">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
                        ${dayExpenses.map(expense => this._renderCardDesktop(expense)).join('')}
                    </div>
                </div>`;
        });
    ...
    // ============================================================
    //  MOBILE RENDER (lista compatta, touch-friendly)
    // ============================================================
    _renderMobile(groupedByDate) {
        ...
        Object.entries(groupedByDate).forEach(([dateStr, dayExpenses]) => {
            ...
            // Date header compatto
            html += `
                <div class="animate-fadeIn">
                    <div class="flex items-center justify-between mb-3 px-1">
                        <div class="flex items-center gap-3">
                            <div class="w-11 h-11 rounded-2xl ${isToday ? 'bg-indigo-600 shadow-lg shadow-indigo-500/20' : 'bg-slate-800 border border-slate-700'} flex flex-col items-center justify-center">
                                <span class="text-[8px] ${isToday ? 'text-indigo-200' : 'text-slate-500'} font-black uppercase leading-none">${isToday ? 'Oggi' : date.toLocaleDateString('it-IT', { weekday: 'short' })}</span>
                                <span class="text-base ${isToday ? 'text-white' : 'text-slate-200'} font-black leading-none">${date.getDate()}</span>
                            </div>
                            <div>
                                <p class="text-sm font-bold text-white leading-tight">${date.toLocaleDateString('it-IT', { weekday: 'long' })}</p>
                                <p class="text-[11px] text-slate-500 font-medium">${date.toLocaleDateString('it-IT', { month: 'long', year: 'numeric' })}</p>
                            </div>
                        </div>
                        <div class="px-2.5 py-1 rounded-lg bg-slate-800/80 border border-slate-700/50 text-[11px] font-black ${dayTotal >= 0 ? 'text-emerald-400' : 'text-rose-400'}">
                            ${dayTotal >= 0 ? '+' : '-'}${Helpers.formatCurrency(Math.abs(dayTotal))}
                        </div>
                    </div>

                    <div class="space-y-2">
                        ${dayExpenses.map(expense => this._renderCardMobile(expense)).join('')}
                    </div>
                </div>`;
        });
    ...
    _renderCardMobile(expense) {
        const category = window.Categories?.getById(expense.category) || { icon: '📦', name: 'Altro' };
        const isIncome = expense.type === 'income';
        const absAmount = Math.abs(parseFloat(expense.amount));

        return `
            <div class="flex items-center gap-3 bg-slate-800/40 border border-slate-700/40 rounded-2xl px-4 py-3 active:bg-slate-800/80 active:scale-[0.98] transition-all cursor-pointer"
                 onclick="ExpenseModals.showDetail('${expense.id}')">
                
                <div class="w-10 h-10 shrink-0 rounded-xl bg-slate-900/60 flex items-center justify-center text-lg border border-white/5">
                    ${category.icon}
                </div>

                <div class="flex-1 min-w-0">
                    <h4 class="text-sm font-bold text-white truncate leading-tight">${Helpers.escapeHtml(expense.description)}</h4>
                    <p class="text-[10px] font-semibold text-slate-500 uppercase tracking-wide">${category.name}</p>
                </div>

                <div class="text-right shrink-0 pl-2">
                    <div class="text-sm font-black ${isIncome ? 'text-emerald-400' : 'text-rose-400'} tracking-tight">
                        ${isIncome ? '+' : '-'}${Helpers.formatCurrency(absAmount)}
                    </div>
                </div>
            </div>`;
    ...
    renderCategoryFilters() {
        ...
        return categories.map(cat => `
            <button onclick="ExpenseFilters.setCategory('${cat.id}'); Expenses.render();"
                    class="${isMob ? 'px-3 py-1.5 rounded-lg text-[10px]' : 'px-4 py-2 rounded-xl text-[11px]'} transition-all font-black uppercase tracking-[0.1em] flex items-center gap-1.5 
                ...
                <span class="${isMob ? 'text-sm' : 'text-base'}">${cat.icon}</span><span>${cat.name}</span>
            </button>`).join('');
    }
};
